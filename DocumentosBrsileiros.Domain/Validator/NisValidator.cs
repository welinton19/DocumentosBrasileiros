...
public class NisValidator : IDocumentoValidator
{
    public TipoDocumento TipoDocumentoSuportado => TipoDocumento.NIS;
    ...
    public ResultadoValidacao Validar(Documento documento)
    {
        ...
        if (string.IsNullOrEmpty(doc) || doc.Length != 11)
            return ResultadoValidacao.Criar(false, documento.Valor, TipoDocumentoSuportado, null,
                new List<string> { "NIS deve conter 11 dígitos." });

        if (!doc.All(char.IsDigit))
            return ResultadoValidacao.Criar(false, documento.Valor, TipoDocumentoSuportado, null,
                new List<string> { "NIS deve conter apenas números." });
        ...
        if (digito != int.Parse(doc[10].ToString()))
            return ResultadoValidacao.Criar(false, documento.Valor, TipoDocumentoSuportado, null,
                new List<string> { "NIS com dígito verificador inválido." });
